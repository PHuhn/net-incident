...
import { Component, OnInit, AfterViewInit, OnChanges, Input, Output, ViewChild, EventEmitter, ElementRef, SimpleChanges } from '@angular/core';
...
export class NetworkLogGridComponent implements OnInit, AfterViewInit, OnChanges {
	//
	// --------------------------------------------------------------------
	// Data declaration.
	// Local variables
	...
	ngAfterViewInit() {
		...
		// https://stackoverflow.com/questions/44070732/
		//  angular-4-expressionchangedafterithasbeencheckederror-expression-has-changed
		...
		Observable.interval( 100 ).takeWhile( val => cnt < 4 ).subscribe( val => {
			cnt++;
			if( this.logLevel >= 4 )
				console.log ( `${this.codeName}.ngAfterViewInit: ${val}, ${ret}.` );
	...
	handleRowUnSelect( event ) {
		...
		if( !this.disabled ) {
			...
			let cnt: number = this.networkIncident.networkLogs.reduce( (previousCnt, currentObject) => {
